...
def get_cached_services() -> Tuple[Any, Any, Any, Any, Any]:
    ...
    # Create metadata fetcher with OpenSearch support
...
def setup_environment():
    """Setup environment and verify all service dependencies."""
...
def fetch_daily_papers(**context):
    ...
    OpenSearch indexing is handled by the dedicated ``index_papers_to_opensearch``
    task that runs after this one in the DAG.
    """
    ...
    try:
        ...
        results = asyncio.run(
            run_paper_ingestion_pipeline(
                ...
                max_results=None,
                process_pdfs=True,
                index_to_opensearch=False,  # Dedicated task handles indexing
            )
        )
...
def index_papers_to_opensearch(**context):
    ...
    This is the single dedicated task for OpenSearch indexing — reads papers
    stored today by ``fetch_daily_papers`` and indexes them via the client.
    """
    ...
    try:
        ...
        with database.get_session() as session:
            ...
            logger.info(f"Found {len(paper_rows)} papers from today's run to index")
            papers = paper_rows  # hold reference outside session for logging
            ...
            for paper_row in paper_rows:
                try:
                    ...
                    paper_doc = {
                        ...
                        "title": paper.title,
                        "authors": (
                            ", ".join(paper.authors) if isinstance(paper.authors, list) else str(paper.authors)
                        ),
                    ...
                    failed_count += 1
                    logger.error(f"Error indexing paper row {paper_row.id}: {e}")
...
def generate_daily_report(**context):
    """Generate a daily processing report aggregating all task results."""
    ...
    try:
        ...
        logger.info(f"PDF failures skipped: {report['processing']['pdf_failures_skipped']}")
