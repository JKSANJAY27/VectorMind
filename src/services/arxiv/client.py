...
_ARXIV_HEADERS = {"User-Agent": "VectorMind/1.0 (mailto:admin@example.com)"}
_RETRY_DELAYS = [30, 60, 120]  # seconds to wait on successive 429s
...
class ArxivClient:
    ...
    async def fetch_papers(
        ...
        try:
            ...
            xml_data = await self._make_request(url)
            papers = self._parse_response(xml_data)
            logger.info(f"Fetched {len(papers)} papers")
            return papers

        except (ArxivAPITimeoutError, ArxivAPIException):
            raise
    ...
    async def _make_request(self, url: str) -> str:
        """
        Make an HTTP GET request to arXiv with rate limiting and 429 retry-backoff.
        ...
        Raises:
            ArxivAPITimeoutError: On timeout
            ArxivAPIException: On HTTP errors after all retries exhausted
        """
        # Enforce inter-request rate limit
        ...
        for attempt, retry_delay in enumerate([0] + _RETRY_DELAYS):
            if retry_delay:
                logger.warning(f"arXiv 429 rate limit hit. Waiting {retry_delay}s before retry {attempt}/{len(_RETRY_DELAYS)}...")
                await asyncio.sleep(retry_delay)
            # Use a longer timeout on retries to survive TCP-level throttling
            effective_timeout = self.timeout_seconds if attempt == 0 else self.timeout_seconds * 3
            try:
                async with httpx.AsyncClient(
                    ...
                    if response.status_code == 429:
                        if attempt < len(_RETRY_DELAYS):
                            continue  # will sleep on next loop iteration
                        raise ArxivAPIException(f"arXiv API returned error 429 after {len(_RETRY_DELAYS)} retries")
                    response.raise_for_status()
                    return response.text
            except httpx.TimeoutException as e:
                if attempt < len(_RETRY_DELAYS):
                    logger.warning(f"Timeout on attempt {attempt + 1}, will retry after backoff...")
                    continue
        ...
        raise ArxivAPIException("arXiv API request failed after all retries")
    ...
    async def _download_with_retry(self, url: str, path: Path, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient(timeout=30.0, headers={"User-Agent": "VectorMind/1.0 (mailto:admin@example.com)"}) as client:
