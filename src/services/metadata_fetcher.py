...
class MetadataFetcher:
    ...
    2. Download PDFs with caching and concurrency control
    3. Parse PDFs with Docling
    4. Store complete paper data in PostgreSQL
    5. (Optional) Index papers to OpenSearch for BM25 search
    """
    ...
    async def fetch_and_process_papers(
        ...
        try:
            ...
            logger.info(
                ...
                f"{results['papers_fetched']} fetched, {results['pdfs_downloaded']} PDFs, "
                f"{results['papers_stored']} stored, {results['papers_indexed']} indexed, "
    ...
    async def _process_pdfs_batch(self, papers: List[ArxivPaper]) -> Dict[str, Any]:
        ...
        - Parsing starts immediately after each download completes
        - Multiple PDFs can be parsing while others are still downloading

        :param papers: List of ArxivPaper objects
        :returns: Dict with downloaded/parsed counts and parsed_papers map
        :rtype: Dict[str, Any]
        """
        results: Dict[str, Any] = {
            "downloaded": 0,
        ...
        logger.info(f"Starting async PDF pipeline for {len(papers)} papers...")
        logger.info(f"Concurrent downloads: {self.max_concurrent_downloads}, parsing: {self.max_concurrent_parsing}")
        ...
        if results["download_failures"]:
            ...
            results["errors"].extend([f"Download failed: {aid}" for aid in results["download_failures"]])
        if results["parse_failures"]:
            logger.warning(f"Parse failures: {len(results['parse_failures'])}")
            results["errors"].extend([f"PDF parse failed: {aid}" for aid in results["parse_failures"]])
    ...
    async def _download_and_parse_pipeline(
        ...
        :returns: Tuple of (download_success: bool, parsed_paper: Optional[ParsedPaper])
        :rtype: tuple
        """
        ...
        try:
            ...
            async with parse_semaphore:
                ...
                if pdf_content:
                    ...
                    logger.debug(f"Parse complete: {paper.arxiv_id} — {len(pdf_content.raw_text)} chars")
                else:
    ...
    def _store_papers_to_db(
        ...
        :param papers: List of ArxivPaper metadata
        :param parsed_papers: Dictionary of parsed PDF content by arxiv_id
        :param db_session: Database session
        :type papers: List[ArxivPaper]
        :type parsed_papers: Dict[str, ParsedPaper]
        :type db_session: Session
        :returns: Number of papers stored successfully
        :rtype: int
        """
        ...
        for paper in papers:
            try:
                ...
                if parsed_paper:
                    ...
                    logger.debug(
                        f"Storing {paper.arxiv_id} with parsed content "
                        f"({len(parsed_content.get('raw_text', '') or '')} chars)"
                    )
                else:
                    paper_data.update({"pdf_processed": False, "parser_metadata": {"note": "PDF not available"}})
                    logger.debug(f"Storing {paper.arxiv_id} with metadata only")
    ...
    def _index_papers_to_opensearch(
        ...
        """Index papers to OpenSearch for BM25 full-text search.

        :param papers: List of ArxivPaper metadata
        :param parsed_papers: Dictionary of parsed PDF content by arxiv_id
        :type papers: List[ArxivPaper]
        :type parsed_papers: Dict[str, ParsedPaper]
        :returns: Number of papers successfully indexed
        :rtype: int
        """
        indexed_count = 0
...
def make_metadata_fetcher(
    ...
    """Factory function to create a MetadataFetcher instance.
